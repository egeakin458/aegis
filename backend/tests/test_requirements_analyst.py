...
class TestDDCSchema:
    ...
    def patch_use_ddc(self, monkeypatch):
        monkeypatch.setattr("app.agents.requirements_analyst.settings.use_ddc", True)
    ...
    def test_ddc_prompt_contains_schema(self):
        agent = self._make_agent()
        assert "CustomerConfigV2" in agent.system_prompt or "ddc-v1" in agent.system_prompt

    def test_ddc_prompt_contains_id_instruction(self):
        agent = self._make_agent()
        assert "id" in agent.system_prompt.lower()

    def test_ddc_user_prompt_free_text(self):
        agent = self._make_agent()
    ...
    def test_ddc_user_prompt_switches_from_legacy_path(self):
        agent = self._make_agent()
