...
When settings.use_ddc=True, receives CustomerConfigV2 and maps
entities → DataModels, use_cases → APIEndpoints (with feature_id threading).
"""
...
"api_endpoints" — list of endpoint objects, required, minimum 1. Each has:
  ...
  "response" — string.
  "feature_id" — string or null. Set to the use_case.id this endpoint implements when available.
...
class SolutionArchitect(BaseAgent):
    ...
    Receives FinalizedConfig (legacy) or CustomerConfigV2 (DDC) and produces
    a complete TechnicalDesign document.
    """
